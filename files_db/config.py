...
def main():
    ...
    # Try the configured (shared drive) path first. On this shared machine,
    # the G: mapping isn't always reliably mounted every session — rather
    # than losing all the query work we just did to a crash, fall back to
    # writing locally so you always get a usable file, and tells you
    # clearly that the shared-drive copy still needs to be done manually.
    output_path = config.OUTPUT_HTML
    try:
        with open(output_path, "w", encoding="utf-8") as f:
            f.write(html)
    except (FileNotFoundError, OSError) as e:
        fallback_path = os.path.join(os.getcwd(), os.path.basename(config.OUTPUT_HTML))
        print(f"WARNING: could not write to {output_path} ({e}).")
        print(f"         Falling back to local path instead: {fallback_path}")
        print(f"         Check that the G: drive is mapped on this machine, then")
        print(f"         copy the file to the shared path yourself if needed.")
