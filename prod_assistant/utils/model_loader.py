...
class ApiKeyManager:
    ...
    def __init__(self):
        ...
        # 🔒 Hard guard against Windows `set VAR=...` poisoning
        endpoint = self.api_keys.get("ASTRA_DB_API_ENDPOINT")
        if endpoint and endpoint.lower().startswith("set "):
            raise RuntimeError(
                "❌ ASTRA_DB_API_ENDPOINT is polluted with Windows 'set' syntax. "
...
class ModelLoader:
    ...
    def load_embeddings(self):
        """
        Load HuggingFace transformer embeddings (local, no API keys).
        """
        try:
            model_name = (
                self.config
